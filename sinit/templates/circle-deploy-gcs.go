...
// CircleDeployGCS returns the circle ci deploy job and workflow for pushing a build artifact to Google Cloud Storage Buckets
func CircleDeployGCS() (t *template.Template) {
	const raw = `  
  deploy:
    docker:
...
workflows:
  ...
  build-deploy:
	...
	t, err := template.New("circle-deploy-gcs.yml").Parse(raw)
	if err != nil {
		panic(err)
	}
